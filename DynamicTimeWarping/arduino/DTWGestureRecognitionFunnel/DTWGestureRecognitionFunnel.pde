...
void processSample(int x, int y, int z) {
  ...
  if (sampleCount > (sampleDataLength - 1)) {
    ...
    //Serial.print(gesture);
    //Serial.println();
    gestureData[0] = gesture;
    Firmata.sendSysex(GESTURE, 1, gestureData);
  }
}
...
int detectGesture() {
  ...
  unsigned int minimumDistance = MIN_DISTANCE;
...
byte quantizeAccelData(int accData) {
  ...
  // represents the value of the constant corresponding to the acceleration of gravity
  ...
  // based on 3.3v sensor
  const int plus2G = 724;
  const int plus1G = 618;
  const int zeroG = 512;
  const int minus1G = 406;
  const int minus2G = 300;  

  // 量子化した値
  byte quantized = 0;

  // 加速度センサの値に従って非線形に量子化する
  // non-linear quantizing according to the value of the accelerometer
  if (plus2G < accData) {
    // 2Gより大きければ16
    quantized = 16;
  }
...
void waitForAction() {
  // 初期状態の各軸の値
  // the initial value of each axis
